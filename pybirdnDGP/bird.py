...
from pybirdnDGP.common import co, mu
...
class Bird(object):
    ...
    def __init__(self, cosmology=None, with_bias=True, eft_basis='eftoflss', with_stoch=False, with_nnlo_counterterm=False, with_shapefit=False, co=co):
        ...
        self.with_shapefit = with_shapefit
    ...
    def setcosmo(self, cosmo):
        ...
        # if self.with_shapefit:
        #     self.Pin = self.setShapefit(self.Pin)
        ...
        self.DA = cosmo["DA"]
        self.H = cosmo["H"]
        self.m = cosmo["m_shapefit"]
        self.alpha_par  = cosmo["alpha_par"]
        self.alpha_perp = cosmo["alpha_perp"]

        if self.co.with_input_kernelfunc:
            self.co.exact_time = False
            self.G1,self.Y1,self.G1t,self.V12t = cosmo['Kernel_func']
    ...
    def setBias(self, bias):
        ...
        if self.co.exact_time or self.co.with_input_kernelfunc:
        ...
        if self.co.halohalo:
            ...
            if self.with_bias: # evaluation with biases specified
                ...
                if self.co.Nloop == 12: self.bloop = np.array([1., b1, b2, b3, b4, b1 * b1, b1 * b2, b1 * b3, b1 * b4, b2 * b2, b2 * b4, b4 * b4])
