...
from pybirdnDGP.nonlinear import NonLinear
from pybirdnDGP.nnlo import NNLO_higher_derivative, NNLO_counterterm
from pybirdnDGP.resum import Resum
from pybirdnDGP.projection import Projection
from pybirdnDGP.greenfunction import GreenFunction
from pybirdnDGP.fourier import FourierTransform
from pybirdnDGP.matching import Matching
...
class Correlator(object):
    ...
    def __init__(self, config_dict=None, load_engines=True):
        ...
        self.cosmo_catalog = {
            ...
            "f": Option("f", float,
                ...
                default=None) ,
            "sigma8": Option("sigma8", float,
                description="Amplitude rescaling (fiducial sigma8 is normalized to 1), i.e, Plin(k) = sigma8^2*Plin(k). Default: sigma8=1.",
                default=None) ,
            "h": Option("h", float,
                description="hubble parameter for NNLO.",
                default=None) ,
            ...
            "DA": Option("DA", float,
                ...
                default=None) ,
            "alpha_par": Option("alpha_par", float,
                description="Parallel BAO scaling parameter. To specify if 'with_shapefit' is True.",
                default=None) ,
            "alpha_perp": Option("alpha_perp", float,
                description="Perpendicular BAO scaling parameter. To specify if 'with_shapefit' is True.",
                default=None) ,
            "m_shapefit": Option("m_shapefit", float,
                description="The shapefit parameter m. To specify if 'with_shapefit' is True.",
                default=None) ,
            "z": Option("z", float,
            ...
            "Omega0_rc": Option("Omega0_rc", float,
                ...
                default=None) ,
            "screening": Option("screening", bool,
                description="Turn on/off screening for nDGP. To specify if \'with_exact_time\' is True.",
                default=True) ,
            "H0": Option("H0", float,
        ...
        self.c_catalog = {
            ...
            "z2": Option("z2", float,
                ...
                default=None) ,
            "with_input_kernelfunc": Option("with_input_kernelfunc", bool,
                description="Use user provided kernel functions G1, Y1, G1t, V12t.",
                default=False) ,
            ...
            "binsize": Option("binsize", float,
                ...
                default=None) ,
            "with_ap": Option("with_AP", bool,
                description="Apply Alcock Paczynski effect. ",
                default=False) ,
            "with_shapefit": Option("with_Shapefit", bool,
                description="Apply Shapefit scaling. ",
                default=False) ,
            "H_fid": Option("H_fid", float,
                description="Hubble parameter by H_0. To specify if \'with_ap\' is True and 'with_shapefit' is False.",
                default=None) ,
            "D_fid": Option("D_fid", float,
                description="Angular distance times H_0. To specify if \'with_ap\' is True and 'with_shapefit' is False.",
                default=None) ,
            "sigma8_fid": Option("sigma8_fid", float,
                description="Fiducial value of sigma8. To specify if 'with_shapefit' is True and sigma8 is not None.",
                default=None) ,
            "k_piv_shapefit":  Option("k_piv_shapefit", float,
                description="The pivot scale [h/Mpc] for Shapefit scaling. ",
                default=0.03) ,
            "a_shapefit":  Option("a_shapefit", float,
                description="A nuisance parameter in Shapefit scaling",
                default=0.6) ,
    ...
    def set_shapefit(self):
        kmode = self.cosmo["kk"]
        factor_m = self.cosmo["m_shapefit"]
        factor_a = self.c["a_shapefit"]
        factor_kp = self.c["k_piv_shapefit"]
    ...
    def compute(self, cosmo_dict=None, cosmo_module=None, cosmo_engine=None, correlator_engine=None, do_core=True, do_survey_specific=True):
        ...
        if self.c["with_shapefit"]:
            self.cosmo["pk_lin"] *= self.set_shapefit()
            if self.cosmo["sigma8"] is not None: self.cosmo["pk_lin"] *= (self.cosmo["sigma8"]/self.c["sigma8_fid"])**2
            if self.cosmo["pk_lin_2"] is not None: 
                self.cosmo["pk_lin_2"] *= self.set_shapefit()
                if self.cosmo["sigma8"] is not None: self.cosmo["pk_lin_2"] *= (self.cosmo["sigma8"]/self.c["sigma8_fid"])**2

        if do_core:
            self.bird = Bird(self.cosmo, with_bias=self.c["with_bias"], eft_basis=self.c["eft_basis"], with_stoch=self.c["with_stoch"], with_nnlo_counterterm=self.c["with_nnlo_counterterm"], with_shapefit=self.c["with_shapefit"], co=self.co)
            if self.c["with_nnlo_counterterm"]: # we use smooth power spectrum since we don't want spurious BAO signals
                if self.cosmo["Psmooth"] is None:
                    self.cosmo["kk"], self.cosmo["Psmooth"], self.cosmo["pk_lin"] = self.get_smooth_wiggle_resc(self.cosmo["kk"], self.cosmo["pk_lin"],self.cosmo["h"])
    ...
    def __load_engines(self, load_engines=True):
        ...
        self.co = Common(Nl=self.c["multipole"], kmax=self.c["kmax"], km=self.c["km"], kr=self.c["kr"], nd=self.c["nd"],
    ...
    def __is_cosmo_conflict(self):
        ...
        if self.c["with_nnlo_counterterm"] and self.cosmo["h"] is None:
            raise Exception("Please specify the hubble parameter for nnlo_counterterm \'D\'.")
        ...
        if self.c["with_shapefit"] and (self.cosmo["m_shapefit"] is None or self.cosmo["alpha_par"] is None or self.cosmo["alpha_perp"] is None):
            raise Exception("You asked to do the Shapefit analysis. Please specify \'m_shapefit\', \'alpha_par\', and \'alpha_perp\'. ")

        if self.c["with_shapefit"] and (self.cosmo["sigma8"] is not None and self.c["sigma8_fid"] is None):
            raise Exception("You asked to perform Shapefit analysis and vary sigma8. Please provide 'sigma8_fid'.")

        if self.c["with_input_kernelfunc"] and self.cosmo["Kernel_func"] is None:
            raise Exception("You asked to input your kernel functions G1, Y1, G1t, V12t. Please provide them.")
    ...
    def __is_config_conflict(self):
        ...
        if self.c["with_shapefit"] and (self.c["k_piv_shapefit"] is None or self.c["a_shapefit"] is None):
            raise Exception("You asked to perform Shapefit analysis. Please specify \'k_piv_shapefit\' and \'a_shapefit\'. ")

        if self.c["with_shapefit"] and not self.c["with_ap"]:
            raise Exception("You asked to perform Shapefit analysis. Please turn on with_ap.")
    ...
    def get_smooth_wiggle_resc(self,kk, pk, hubble, alpha_rs=1.): # k [h/Mpc], pk [(Mpc/h)**3]
        kp = np.linspace(1.e-7, 7, 2**16)   # 1/Mpc
        ilogpk = interp1d(np.log(kk * hubble), np.log(pk / hubble**3), fill_value="extrapolate") # Mpc**3
        ...
        spk = interp1d(kp, smooth_pk, bounds_error=False)(kk * hubble) * hubble**3 # (Mpc/h)**3
        wpk_resc = interp1d(kp, wiggle_pk, bounds_error=False)(alpha_rs * kk * hubble) * hubble**3 # (Mpc/h)**3 # wiggle rescaling
